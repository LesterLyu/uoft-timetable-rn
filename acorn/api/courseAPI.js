...
class CourseAPI {
  ...
  async enrolledCourses(registrationIndex = 0) {
    const url = 'https://acorn.utoronto.ca/sws/rest/enrolment/course/enrolled-courses';
    const params = this.registrations[registrationIndex].registrationParams;
    await request(url, {qs: params, json: true});
  }
  ...
  async enrollmentCart(registrationIndex = 0) {
    const params = this.registrations[registrationIndex].registrationParams;
    return await request('https://acorn.utoronto.ca/sws/rest/enrolment/plan', {
      qs: {
  ...
  async courseDetail(courseCode, courseSessionCode, sectionCode, registrationIndex = 0) {
    const params = this.registrations[registrationIndex].registrationParams;
    return await request('https://acorn.utoronto.ca/sws/rest/enrolment/course/view', {
      qs: {
        ...params,
        activityApprovedInd: '',
        activityApprovedOrg: '',
